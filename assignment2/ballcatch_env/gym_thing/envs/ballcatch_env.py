...
class ThingBallCatchEnv(gym.Env):
    ...
    def __init__(
        ...
        self.arm_joint_names = [
            "ur10_arm_0_shoulder_pan_joint",
            "ur10_arm_1_shoulder_lift_joint",
            "ur10_arm_2_elbow_joint",
            "ur10_arm_3_wrist_1_joint",
            "ur10_arm_4_wrist_2_joint",
            "ur10_arm_5_wrist_3_joint",
        ]
        self.arm_act_names = [
            "ur10_arm_0_shoulder_pan_act",
            "ur10_arm_1_shoulder_lift_act",
            "ur10_arm_2_elbow_act",
            "ur10_arm_3_wrist_1_act",
            "ur10_arm_4_wrist_2_act",
            "ur10_arm_5_wrist_3_act",
        ]
        self.arm_body_names = [
        ...
        self.action_space = spaces.Box(
            low=np.array([-0.02 * math.pi] * 6, dtype=np.float32),
        ...
        self.observation_space = spaces.Box(
            low=np.array([-2 * math.pi] * (len(self.arm_joint_names) * 2) + [-10] * 9, dtype=np.float32),
        ...
        self.goal = np.array(self.get_optimal_catching_pos())[
            0:6
        ]  # Currently, get_optimal_catching_pos() returns joint states for both arm and base
    ...
    def _reset_sim(self, arm_joints=None, ball_xpos=None, ball_qvel=None):
        if self.mass_std != 0:
            new_mass = self.initial_mass * np.clip(
                np.random.normal(1, self.mass_std, self.initial_mass.shape), 0.5, 1.5
            )
            for i in range(len(self.sim.model.body_mass)):
                self.sim.model.body_mass[i] = new_mass[i]
        if self.act_gain_std != 0:
            new_act_gain = self.initial_act_gain * np.clip(
                np.random.normal(1, self.act_gain_std, self.initial_act_gain.shape), 0.5, 1.5
            )
        ...
        if self.gravity_factor_std != 0:
            new_gravity_factor = self.initial_gravity_factor * np.clip(
                np.random.normal(1, self.gravity_factor_std), 0.5, 1.5
            )
            self.sim.data.xfrc_applied[self.model.body_name2id("ball")][2] = (
                -9.81 * new_gravity_factor
            )  # Add gravity to ball
        ...
        if self.online:
            new_ball_qvel = (
                ball_qvel
                if ball_qvel is not None
                else (self.initial_ball_qvel * np.clip(np.random.normal(1, 0.1, self.initial_ball_qvel.shape), 0, 2))
            )  # Add noise
            new_ball_qpos = (
                ball_xpos
                if ball_xpos is not None
                else (self.initial_ball_qpos * np.clip(np.random.normal(1, 0.1, self.initial_ball_qpos.shape), 0, 2))
            )  # Add noise
    ...
    def _set_action_inv_kin(self, action):
        ...
        rotation_mat = euler2mat(action[3:6])
    ...
    def _is_success(self, achieved_goal=None, desired_goal=None):
        ...
        return self.is_point_in_cylinder(
            self.sim.data.get_body_xpos("cup"),
            self.sim.data.get_body_xpos("ur10_arm_ee_link"),
            0.2,
            0.05,
            self.sim.data.get_body_xpos("ball"),
    ...
    def _random_pose_in_sphere(self, radius=1.0, xpos=0.0, ypos=0.0, zpos=0.0, zscale=0.8):
    ...
    def goal_distance_xyz(self, achieved_goal, desired_goal):
        ...
        rotation_distance = np.arccos(
            np.square(achieved_goal[3:].dot(desired_goal[3:])) - 1
        )  # Formula for rotation between two quaternions
    ...
    def compute_reward(self, achieved_goal, desired_goal, info):
        ...
        if self.temp_penalty_type == 0:
            ...
            rew = 0.1 * (-self.goal_distance_joint(achieved_goal[0:6], desired_goal)) + 0.01 * (
                -np.linalg.norm(achieved_goal[6:12])
            )
        elif self.temp_penalty_type == 2:
            rew = 0.1 * (-self.goal_distance_joint(achieved_goal[0:6], desired_goal)) + 0.001 * (
                -np.linalg.norm(self.sim.data.qacc[5:11])
            )
        elif self.temp_penalty_type == 3:
            rew = (
                0.1 * (-self.goal_distance_joint(achieved_goal[0:6], desired_goal))
                + 0.01 * (-np.linalg.norm(achieved_goal[6:12]))
                + 0.001 * (-np.linalg.norm(self.sim.data.qacc[5:11]))
            )
    ...
    def get_optimal_ball_trajectory(self):
        ...
        cup_y_axis *= np.clip(np.random.normal(1, 0.2, cup_y_axis.shape), 0, 2)  # Add noise
    ...
    def is_point_in_cylinder(
        self, pt_1: np.array, pt_2: np.array, length_squared: float, radius_squared: float, pt_test: np.array
    ):
        d_pt = pt_2 - pt_1
