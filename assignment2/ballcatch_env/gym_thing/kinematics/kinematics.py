...
def calibration_matrix(offsets):
    ...
    T_cal = [
        [
            cos(rot_z) * cos(rot_y),
            -sin(rot_z) * cos(rot_x) + cos(rot_z) * sin(rot_y) * sin(rot_x),
            sin(rot_z) * sin(rot_x) + cos(rot_z) * sin(rot_y) * cos(rot_x),
            trans_x,
        ],
        [
            sin(rot_z) * cos(rot_y),
            cos(rot_z) * cos(rot_x) + sin(rot_z) * sin(rot_y) * sin(rot_x),
            -cos(rot_z) * sin(rot_x) + cos(rot_z) * sin(rot_y) * cos(rot_x),
            trans_y,
        ],
...
def forward_kinematics_arch2tcp(theta):
    ...
    T_a5_2_tcp = np.array(
        [
            [sin(OFFSET_K_TCP_Z_ROTATION), 0, cos(OFFSET_K_TCP_Z_ROTATION), gripper_x_offset],
...
def forward_kinematics_odom2tcp(theta):
    ...
    T_rb2arch = np.array([[1, 0, 0, 0.27], [0, 1, 0, 0.01], [0, 0, 1, 0.653], [0, 0, 0, 1]])
...
def cup_position_given_theta(theta):
    ...
    # constants
    p_W_B_W = np.array([0.27, 0.01, 0.653])
    p_T_C_T = np.array([0, 0, 0.1])
    R_T_C = euler2mat(np.radians([90, 15, 0]))
